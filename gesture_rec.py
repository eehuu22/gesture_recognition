...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        cv2.putText(
            ...
            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA  # 字体、大小、颜色和粗细
        )
        if score >= 90:  # 如果分数大于70，则在右下方打印出GOOD
            cv2.putText(
                image, "Unbelievable!!!", (420, 400),  # 5显示位置
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 5, cv2.LINE_AA  # 字体、大小、颜色和粗细
            )
        elif score>=80 and score<90:
            cv2.putText(
                image, "Great!!", (420, 400),  # 5显示位置
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 5, cv2.LINE_AA  # 字体、大小、颜色和粗细
            )
        elif score<80 and score>70:
            cv2.putText(
                image, "Very Good!", (420, 400),  # 5显示位置
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 5, cv2.LINE_AA  # 字体、大小、颜色和粗细
            )
        elif score<=70 and score>60:
            cv2.putText(
                image, "Not Bad", (420, 400),  # 5显示位置
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 5, cv2.LINE_AA  # 字体、大小、颜色和粗细
            )
        else:
            cv2.putText(
                image, "Try Again", (420, 400),  # 5显示位置
                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 5, cv2.LINE_AA  # 字体、大小、颜色和粗细
            )
        # 如果分数超过 70，切换图像
        # 这里可以修改切换下一个动作的标准
